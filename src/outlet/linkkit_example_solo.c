...
// for demo only
#define PRODUCT_KEY    "replace me "
#define PRODUCT_SECRET "Replace This XxX"

#define DEVICE_NAME    "replace me "
#define DEVICE_SECRET  "replace me with your own value  "
...
#define PROP_POST_FORMAT_TEMP "{\"PowerSwitch\":%d}"
typedef struct {
    int master_devid;
    int cloud_connected;
...
extern char g_key_flag ;
extern gpio_dev_t relay;
extern int relay_gpio_level;  
...
static int user_property_set_event_handler(const int devid, const char *request, const int request_len)
{
    ...
    (request[15]-0x30)?hal_gpio_output_high(&relay):hal_gpio_output_low(&relay);
    relay_gpio_level^=1;
...
void user_post_property(void)
{
    ...
    HAL_Snprintf(property_payload, sizeof(property_payload), "{\"PowerSwitch\": %d}", relay_gpio_level);
