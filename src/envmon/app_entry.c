...
#endif
i2c_dev_t sensor_i2c_master = {
    .port                 = 1,
    .config.address_width = 0,
    .config.freq          = 96000,
    .config.dev_addr      = 0,
};
...
static void wifi_service_event(input_event_t *event, void *priv_data)
{
    ...
    if (!linkkit_started) {
        ...
        print_heap();
#endif
        aos_task_new("linkkit", (void (*)(void *))linkkit_main, NULL, 1024 * 12);
        linkkit_started = 1;
    }
}
...
void linkkit_key_process(input_event_t *eventinfo, void *priv_data)
{
    ...
    if (eventinfo->code == CODE_BOOT) {
	    ...
	    hal_gpio_output_toggle(&control_out);
            do_awss_active();
...
int application_start(int argc, char **argv)
{
    ...
    aos_task_new("dap_open", awss_open_dev_ap, NULL, 4096);
#else
    aos_task_new("netmgr_start", start_netmgr, NULL, 1024*5);
#endif
#endif
    aos_loop_run();
