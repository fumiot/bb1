...
// for demo only
#define PRODUCT_KEY    "replaceitXX"
#define PRODUCT_SECRET "ReplaceitReplace"

#define DEVICE_NAME    "testEnvMon1"
#define DEVICE_SECRET  "replace with one from aliyun iot"
...
float Humi;
char pageBuffer[10];
extern i2c_dev_t sensor_i2c_master ;
...
int linkkit_main(void *paras)
{
    ...
    while (1) {
        ...
        if (ret > 0) {
            ...
            /* build the report payload */
            sprintf(param, PROP_POST_FORMAT_HUMI, humi.h/10.0);
	    printf("\n param len %d\n",strlen(param)+1);
